...
export class StatusService {
  ...
  constructor(private http:HttpClient){}
    public getStatuses(): Observable<Status[]> {
      return this.http.get<Status[]>(`${this.apiServerUrl}`, {withCredentials: true});
    }
    public getStatusWithId(id: number): Observable<Status> {
      return this.http.get<Status>(`${this.apiServerUrl}/${id}`, {withCredentials: true});
    }
    public addStatus(status: StatusDtoInsert): Observable<string> {
      return this.http.post(`${this.apiServerUrl}`, status, { responseType: 'text' , withCredentials: true});
    }
